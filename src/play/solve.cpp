...
public:
    ...
    void set(char letter, bool value = true)
    {
        _bitfield.set(index(letter), value);
    }
private:
    [[nodiscard]] static constexpr int index(char letter)
    {
        Expects(std::isalpha(letter));
...
namespace {
    ...
    auto make_cell_tile_scores(
            board<char> const& board_tiles,
            board<premium> const& premiums,
            letter_values const& letter_scores) -> board<int>
    {
        auto const edge{ssize(board_tiles)};
        board<int> cell_tile_scores(edge);
        
        auto p = coord{};
        for (p[1] = 0; p[1] != edge; ++ p[1])
        {
            for (p[0] = 0; p[0] != edge; ++ p[0])
            {
                auto const letter = board_tiles.cell(p);
                auto const letter_score = letter_scores[letter];
                cell_tile_scores.cell(p) = letter_score;
            }
        }
    ...
    auto calc_cross_score(
//            initial_state const& init,
            board<int> const& cell_tile_scores,
            coord const& part_start,
            coord const& direction,
//            std::pair<int, int> extents)
    {
        ...
        board<premium> premiums) -> std::vector<result>
{
    ...
    auto const qualifying_cells{make_qualifying_cells(tiles)};
    auto const cell_tile_scores{make_cell_tile_scores(tiles, premiums, letter_scores)};

    auto init{make_start_state(
            lexicon, letter_scores,
            premiums, word, letters)};
    step_state step{
            std::begin(word),
            finds,
            rack
    ...
    std::vector<int> line_premiums{edge};
    std::vector<int> cross_scores{edge};

    for (auto bearing{0}; bearing!=2; ++bearing) {
        auto const orthogonal{1-bearing};
        auto const direction{coord{bearing, orthogonal}};
        auto const cross_direction{coord{orthogonal, bearing}};
        
        for (auto cell_and_line = coord{0, 0};
                cell_and_line[1]!=edge;
                ++cell_and_line[1]) {
            auto count_back{0};
            for (cell_and_line[0] = edge-1;
                    cell_and_line[0]>=0;
                    --cell_and_line[0]) {
                auto const pos = coord{
                        cell_and_line[orthogonal],
                        cell_and_line[bearing]
                };
                line_premiums[cell_and_line[0]] = premiums[pos[1]][pos[0]];
                cross_scores[cell_and_line[0]] = calc_cross_score(
                        cell_tile_scores,
                        pos, direction);
//                step.pos = start_pos;
                if (init.qualifying_cells.cell(pos)) {
